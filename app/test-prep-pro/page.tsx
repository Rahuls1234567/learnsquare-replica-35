                            ...
                            <div className="relative bg-white/70 backdrop-blur-3xl rounded-[3rem] p-8 md:p-10 border border-indigo-50 shadow-[0_40px_100px_rgba(99,102,241,0.06)] max-w-lg lg:ml-auto">
                                ...
                                <form className="space-y-5" onSubmit={handleSubmit(onRequestSubmit)}>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <Input {...register("firstName", { required: true })} placeholder="First Name*" className={`h-12 rounded-xl border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 font-bold px-5 focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${errors.firstName ? 'border-red-500/50' : ''}`} />
                                        <Input {...register("lastName", { required: true })} placeholder="Last Name*" className={`h-12 rounded-xl border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 font-bold px-5 focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${errors.lastName ? 'border-red-500/50' : ''}`} />
                                    </div>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <Input {...register("whatsappNo", { required: true })} placeholder="Whatsapp No.*" className={`h-12 rounded-xl border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 font-bold px-5 focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${errors.whatsappNo ? 'border-red-500/50' : ''}`} />
                                        <Input {...register("email", { required: true, pattern: /^\S+@\S+$/i })} type="email" placeholder="Email*" className={`h-12 rounded-xl border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 font-bold px-5 focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${errors.email ? 'border-red-500/50' : ''}`} />
                                    </div>
                                    <Input {...register("collegeName", { required: true })} placeholder="College Name*" className={`h-12 rounded-xl border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 font-bold px-5 focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${errors.collegeName ? 'border-red-500/50' : ''}`} />
                                    <Textarea {...register("message", { required: true })} placeholder="Tell us about your preparation goals..." className={`min-h-[100px] rounded-xl border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 font-bold p-5 focus:ring-2 focus:ring-blue-500/20 transition-all resize-none text-sm ${errors.message ? 'border-red-500/50' : ''}`} />

                                    <div className="flex justify-center mt-6">
                                        <Button type="submit" disabled={mutation.isPending} className="px-10 h-12 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black text-sm hover:shadow-[0_15px_30px_-10px_rgba(37,99,235,0.4)] hover:scale-[1.02] transition-all group/btn shadow-xl border-none disabled:opacity-70 disabled:cursor-not-allowed gap-2">
                                            {mutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                                            {mutation.isPending ? "Submitting..." : "Join Prep Pro"}
