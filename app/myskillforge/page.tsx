...
import { Rocket, BookOpen, Briefcase, Zap, Star, ShieldCheck, Brain, MessageSquare, Code2, PenTool, Github, Database, Calculator, Lightbulb, Atom, Cpu, Globe, Layout, Loader2 } from "lucide-react";
            ...
            <div className={`grid md:grid-cols-2 gap-12 items-center relative z-10`}>
                ...
                <div className={`${reverse ? 'md:order-1 text-right' : ''} space-y-6`}>
                    ...
                    <Button variant="ghost" className={`group/btn p-0 font-black hover:bg-transparent ${reverse ? 'flex-row-reverse gap-2' : 'gap-2'}`} style={{ color: color }}>
                        Explore Phase
                        <span className={`transition-transform duration-300 group-hover/btn:translate-x-2`}>ΓåÆ</span>
                    </Button>
                </div>
            </div>
        </div>
...
const MySkillForgePage = () => {
    ...
    const { register, handleSubmit, reset, control, formState: { errors } } = useForm<EnrolFormData>({
        defaultValues: {
            firstName: '',
            lastName: '',
            mobileNumber: '',
    ...
    const mutation = useMutation({
        mutationFn: async (data: EnrolFormData) => {
            return apiClient.post('/enrol', {
                ...data,
                course: 'MySkillForge - Pre Internship & Virtual Internship Programs'
            });
        },
        onSuccess: () => {
            toast.success("Enrolled successfully!", {
                description: "We've received your enrolment. We'll be in touch soon!",
            });
            reset();
        },
        onError: () => {
    ...
    const onEnrolSubmit = (data: EnrolFormData) => {
                                ...
                                <div className="glass-chrome rounded-[2rem] md:rounded-[3rem] p-6 sm:p-8 md:p-12 border border-white/20 shadow-2xl relative z-10">
                                    ...
                                    <form className="space-y-4" onSubmit={handleSubmit(onEnrolSubmit)}>
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <Input {...register("firstName", { required: true })} placeholder="First Name*" className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white placeholder:text-white/40 font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 transition-all ${errors.firstName ? 'border-red-500/50' : ''}`} />
                                            <Input {...register("lastName", { required: true })} placeholder="Last Name*" className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white placeholder:text-white/40 font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 transition-all ${errors.lastName ? 'border-red-500/50' : ''}`} />
                                        </div>
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <Input {...register("mobileNumber", { required: true })} placeholder="Mobile Number*" className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white placeholder:text-white/40 font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 transition-all ${errors.mobileNumber ? 'border-red-500/50' : ''}`} />
                                            <Input {...register("email", { required: true, pattern: /^\S+@\S+$/i })} type="email" placeholder="Email*" className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white placeholder:text-white/40 font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 transition-all ${errors.email ? 'border-red-500/50' : ''}`} />
                                        </div>
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <Select onValueChange={field.onChange} value={field.value}>
                                                    <SelectTrigger className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 ${errors.course ? 'border-red-500/50' : ''}`}>
                                        ...
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <Controller
                                                ...
                                                render={({ field }) => (
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        <SelectTrigger className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 ${errors.program ? 'border-red-500/50' : ''}`}>
                                            ...
                                            <Controller
                                                ...
                                                render={({ field }) => (
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        <SelectTrigger className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 ${errors.branch ? 'border-red-500/50' : ''}`}>
                                        ...
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <Controller
                                                ...
                                                render={({ field }) => (
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        <SelectTrigger className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 ${errors.yearOfStudy ? 'border-red-500/50' : ''}`}>
                                            ...
                                            <Controller
                                                ...
                                                render={({ field }) => (
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        <SelectTrigger className={`h-14 rounded-2xl border-white/20 bg-white/10 text-white font-bold px-7 focus:ring-2 focus:ring-indigo-500/50 ${errors.semester ? 'border-red-500/50' : ''}`}>
                                        ...
                                        <div className="flex justify-center pt-4">
                                            <Button type="submit" disabled={mutation.isPending} className="w-full h-16 rounded-[2rem] bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black text-xl hover:opacity-90 transition-all shadow-xl active:scale-95 group/btn disabled:opacity-70 disabled:cursor-not-allowed gap-2">
                                                {mutation.isPending && <Loader2 className="w-5 h-5 animate-spin" />}
                                                {mutation.isPending ? "Enrolling..." : "Enrol Now !!"}
