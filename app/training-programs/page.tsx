...
import { Send, Zap, BookOpen, Star, Rocket, Target, X, Loader2 } from "lucide-react";
...
const TrainingProgramsPage = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (data: TrainingRequestFormData) => {
            return apiClient.post('/training-request', data);
        },
        onSuccess: () => {
    ...
    const onRequestSubmit = (data: TrainingRequestFormData) => {
        ...
        >
            ...
            <main className="relative pt-24 pb-20">
                ...
                <div className="container relative z-10 flex flex-col items-center mx-auto px-4 md:px-6">
                        ...
                        >
                            <div className="relative group cursor-default inline-block">
                                ...
                                <div className="relative bg-slate-900/80 border border-indigo-500/30 text-indigo-400 px-6 py-2.5 rounded-full font-black tracking-[0.2em] text-[10px] uppercase shadow-2xl flex items-center gap-2 overflow-hidden">
                                    ...
                                    <span className="relative z-10">EXPERT-LED TRAINING</span>
                                </div>
                            </div>

                            <EditableContent 
                                contentKey="training_hero_heading"
                                description="Training Programs Hero Heading"
                                defaultContent={
                                    <h2 className="font-black tracking-tight flex flex-col items-start drop-shadow-xl w-full">
                                        <span className="text-5xl sm:text-6xl md:text-7xl lg:text-[4.5rem] xl:text-[5.5rem] 2xl:text-[6rem] text-slate-100 mb-2 lg:mb-4 leading-none">Unlock Your</span>
                                        <span className="text-4xl sm:text-5xl md:text-6xl lg:text-[3.5rem] xl:text-[4rem] 2xl:text-[4.5rem] text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-purple-400 pb-2 pr-6 leading-[1.1] italic">Potential</span>
                                    </h2>
                                }
                            />

                            <EditableContent 
                                contentKey="training_hero_desc"
                                description="Training Programs Hero Description"
                                defaultContent={
                            ...
                            <EditableContent 
                                contentKey="training_hero_stats"
                                description="Training Programs Hero Stats"
                                defaultContent={
                                    <div className="pt-4 flex gap-6">
                                        <div className="flex flex-col">
                                            <span className="text-3xl font-black text-white">12+</span>
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Industry Modules</span>
                                        </div>
                                        <div className="w-[1px] bg-slate-800 h-10 self-center" />
                                        <div className="flex flex-col">
                                            <span className="text-3xl font-black text-white">100%</span>
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Placement Focus</span>
                                        </div>
                                    </div>
                                }
                            />
                                ...
                                <form className="space-y-5 relative z-10 flex-grow" onSubmit={handleSubmit(onRequestSubmit)}>
                                    ...
                                    <div className="space-y-2">
                                        <Textarea 
                                            ...
                                            className={`min-h-[100px] bg-white/5 border-white/10 rounded-2xl focus:bg-white/10 transition-all text-white placeholder:text-slate-500 focus-visible:ring-indigo-500/30 font-bold p-5 resize-none text-sm shadow-inner flex pt-4 items-center justify-start ${errors.message ? 'border-red-500/50' : ''}`} 
                                        />
                                    </div>

                                    <div className="pt-4 mt-auto">
                                        <div className="relative group">
                                            <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl blur opacity-30 group-hover:opacity-70 transition duration-1000"></div>
                                            <Button
                                                type="submit"
                                                disabled={mutation.isPending}
                                                className="w-full relative bg-indigo-600 hover:bg-indigo-500 text-white font-black h-14 rounded-2xl transition-all uppercase tracking-widest text-[13px] shadow-lg flex items-center justify-center gap-3 overflow-hidden"
                                            >
                                                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                                                {mutation.isPending ? (
                                                    <Loader2 className="w-5 h-5 animate-spin" />
                                                ) : (
                                                    <span className="flex items-center gap-2">Get Started Now <Rocket className="w-5 h-5" /></span>
                                                )}
                                            </Button>
                                        </div>
                                    </div>
                    ...
                    <div className="mt-48 space-y-20 relative z-10">
                        <EditableContent 
                            contentKey="training_modules_heading"
                            description="Training Programs Modules Heading"
                            defaultContent={
                                <div className="text-center space-y-4">
                                    <span className="text-indigo-400 font-black text-sm uppercase tracking-[0.4em]">Expert Led</span>
                                    <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-purple-400 leading-[1.2] pb-4">
                                        Our Core Modules
                                    </h2>
                                </div>
                            }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                            {programs.map((prog, i) => (
                                <motion.div
                                    ...
                                    className="group relative bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] p-8 border border-white/10 hover:border-indigo-500/30 transition-all duration-500 overflow-hidden shadow-2xl hover:shadow-[0_20px_40px_rgba(99,102,241,0.2)]"
                                >
                                    {/* Texture & Glow */}
                                    ...
                                    <div className="relative z-10 h-full flex flex-col">
                                        <div className="relative overflow-hidden rounded-[2rem] aspect-[16/10] mb-8 bg-slate-900">
                                            <img
                                                src={prog.img}
                                                alt={prog.title}
                                        ...
                                        <div className="space-y-4 flex-grow">
                                            <EditableContent 
                                                contentKey={`training_prog_${prog.id}`}
                                                description={`Training Program ${prog.title}`}
                                                defaultContent={
                                                    <>
                                                        <h3 className="text-2xl font-black text-indigo-300 tracking-tight">{prog.title}</h3>
                                                        <p className="text-slate-300 font-bold text-sm leading-relaxed line-clamp-2 italic">
                                                            {prog.desc}
                                                        </p>
                                                    </>
                                                }
                                        ...
                                        <div className="flex items-center gap-4 mt-8 pt-8 border-t border-white/10">
                                            <Button
                                                ...
                                                className="flex-grow h-12 rounded-full bg-indigo-600 text-white font-black text-xs uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-[0_0_15px_rgba(79,70,229,0.4)] active:scale-95 border-none"
                                            >
                                                Curriculum Details
                                            </Button>
                                            <button className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-white/10 hover:border-white/40 text-white transition-all backdrop-blur-sm">
                    ...
                    <Dialog open={!!selectedProgram} onOpenChange={(open) => !open && setSelectedProgram(null)}>
                        <DialogContent className="max-w-5xl p-0 overflow-hidden border border-white/10 bg-slate-900/95 backdrop-blur-2xl rounded-2xl shadow-2xl">
                            {selectedProgram && (
                                <div className="flex flex-col md:flex-row min-h-[500px] max-h-[90vh] overflow-y-auto md:overflow-hidden">
                                    {/* Left Side: Image */}
                                    <div className="md:w-1/2 bg-slate-950 relative p-4 flex items-center justify-center min-h-[300px] md:min-h-full">
                                    ...
                                    {/* Right Side: Content */}
                                    <div className="md:w-1/2 p-8 md:p-12 relative overflow-y-auto text-white">
                                        <button
                                            onClick={() => setSelectedProgram(null)}
                                            className="absolute top-6 right-6 p-2 rounded-full hover:bg-white/10 transition-all text-slate-400 hover:text-white z-20"
                                        >
                                            <X className="w-6 h-6" />
                                        </button>
                                        ...
                                        <div className="space-y-8">
                                            <div>
                                                <h2 className="text-3xl font-black text-indigo-100 mb-2">{selectedProgram.title}</h2>
                                            </div>

                                            <div className="space-y-8">
                                                {selectedProgram.details.map((section: any, sIdx: number) => (
                                                    <div key={sIdx}>
                                                        <h3 className="text-xl font-black text-indigo-300 mb-4">{section.subtitle}</h3>
                                                        <ul className="space-y-2">
                                                            {section.items.map((item: string, iIdx: number) => (
                                                                <li key={iIdx} className="flex items-start gap-2 text-slate-300 font-bold text-sm">
